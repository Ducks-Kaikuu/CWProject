...
//
//----------------------------------------------------------------------//
bool USNDataAssetManager::SetupDLCContents(){
	...
	for(auto& Module:ModuleDescriptors){
		...
		if(DLCInterface != nullptr){
			// アセット管理用のアセットを取得
			TSoftClassPtr<USNDataAssetManager> AssetManagerSoftClassPtr = DLCInterface->GetAssetManagerSoftClassPtr();
			...
			if(AssetManagerSoftClassPtr.IsNull() == false){
				...
				// 管理クラスを生成
				USNDataAssetManager* Manager = NewObject<USNDataAssetManager>(this, AssetManagerClass);
					...
					if(ContentAsset == nullptr){
						...
						for(auto& Class:Contents.Value->ContentsClassList){
							...
							if(ContentAsset->ContentsClassList.Find(Class.Key) != nullptr){
								// クラスの情報を上書きするログを出力
								SNPLUGIN_ASSERT(TEXT("Duplicate Class names : [%s] : %s"), *Module.ToString(), *Class.Key.ToString());
								// クラスの情報を上書き
